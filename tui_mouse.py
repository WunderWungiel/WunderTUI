...
def menu(options, text=None, args=None, custom_text=None, width=38, space_left=9):
    ...
    while True:
        ...
        if key == "down":
            ...
            current_chosen -= 1
        elif isinstance(key, str) and key.isnumeric() or key == "enter":
            if key == "enter":
                user_choose = current_chosen
            elif key.isnumeric() and int(key) in options_integers:
                key = int(key)
                current_chosen = key
                user_choose = key
            else:
        ...
        elif isinstance(key, list) and key[0] == "mouse":
            x, y = key[1], key[2]
            positions = {}
            for i in options_integers:
                i_x_1 = space_left + 4 + len(str(i))
                i_x_2 = i_x_1 + len(options_names[options_integers.index(i)])

                i_y = 7+i

                positions[i] = [range(i_x_1, i_x_2), i_y]

            for key, value in positions.items():
                if x in value[0] and y == value[1]:
                    current_chosen = key
                    user_choose = key
                    if args:
                        user_choose_name = options_names[options_integers.index(user_choose)]
